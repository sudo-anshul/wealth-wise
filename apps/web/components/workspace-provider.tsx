...
type Context={state:Workspace;mode:SourceMode;basePath:'/demo'|'/app';ready:boolean;pending:boolean;error:string|null;dispatch:(command:Command)=>Promise<void>;resetDemo:()=>void;reload:()=>Promise<void>};
...
export function WorkspaceProvider({mode,children}:{mode:SourceMode;children:ReactNode}){
  ...
  function resetDemo(){if(mode!=='demo')return;const next=createDemoWorkspace();try{localStorage.setItem(storageKey,JSON.stringify(next));update(next);setError(null);}catch{setError('Browser storage is unavailable. Allow local storage to save demo changes.');}}
